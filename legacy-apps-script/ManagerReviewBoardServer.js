// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  MANAGER REVIEW BOARD — SERVER FUNCTIONS  v3.1                        ║
...
function managerApproveTicket(data) {
  try {
    ...
    var origForDesc = getMasterLogFieldForTicket_(data.ticketNo, ML.DESCRIPTION);
    if (origForDesc) mlRow[ML.DESCRIPTION   - 1] = origForDesc;
...
function managerVerifyTicket(data) {
  try {
    ...
    mlRow[ML.UPDATED_BY    - 1] = byEmail;  // ITEM 6D — actual email, not 'Manager'
    mlRow[ML.TRACKER_GROUP - 1] = getDeptGroup_(getMasterLogFieldForTicket_(data.ticketNo, ML.DEPT));
    mlRow[ML.LINE_NO       - 1] = getMasterLogFieldForTicket_(data.ticketNo, ML.LINE_NO) || '';
    mlSh.appendRow(mlRow);

    removeTicketFromSheet_(ss, SH.OPEN, data.ticketNo);
    moveTicketToClosed_(ss, data.ticketNo, {
      dept: '', priority: '', verifiedBy: data.verifiedBy,
      updatedBy: byEmail,
...
function managerReassignTicket(data) {
  try {
    var ss      = SpreadsheetApp.getActiveSpreadsheet();
    var now     = new Date();
    var byEmail = getCurrentUserInfo().email || 'Manager';
    ...
    mlRow[ML.TICKET_NO   - 1] = data.ticketNo;
    mlRow[ML.TIMESTAMP   - 1] = formatTimestamp_(now);
    mlRow[ML.ACTION      - 1] = 'REASSIGNED';
    mlRow[ML.ASSIGNED_TO - 1] = data.assignedTo;
    mlRow[ML.UPDATED_BY  - 1] = byEmail;
    mlRow[ML.TRACKER_GROUP-1] = getDeptGroup_(getMasterLogFieldForTicket_(data.ticketNo, ML.DEPT));
    mlRow[ML.LINE_NO     - 1] = getMasterLogFieldForTicket_(data.ticketNo, ML.LINE_NO) || '';
    mlSh.appendRow(mlRow);

    var sheetsToUpdate = [SH.OPEN, SH.WAITING];
    DEPT_TRACKERS.forEach(function(dt) { sheetsToUpdate.push(dt.name); });

    sheetsToUpdate.forEach(function(shName) {
      var sh = ss.getSheetByName(shName);
      if (!sh || sh.getLastRow() < 5) return;
      var tickets = sh.getRange(5, TK.TICKET_NO, sh.getLastRow() - 4, 1).getValues();
      for (var i = 0; i < tickets.length; i++) {
        if (String(tickets[i][0] || '').trim() === data.ticketNo) {
          sh.getRange(i + 5, TK.ASSIGNED_TO).setValue(data.assignedTo);
          sh.getRange(i + 5, TK.LAST_UPDATED).setValue(formatTimestamp_(now));
          break;
        }
      }
    });

    logTicketHistory(data.ticketNo, TH_EVENTS.ASSIGNED, '', '', data.assignedTo,
      'Reassigned by manager to: ' + data.assignedTo);

    return { success: true };
  } catch(e) {
    Logger.log('managerReassignTicket error: ' + e.message);
    return { success: false, error: e.message };
  }
}
